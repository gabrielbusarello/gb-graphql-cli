 ...
 * mutation
 */
createFile.prototype.mutation = (auth) => {
    const mutation = `// Here you can add yours schemas.
${auth ? 'import { tokenMutations } from "./resources/token/token.schema";' : ''}
 ...
 * schema
 */
createFile.prototype.schema = (auth) => {
...
${auth ? 'import { tokenTypes } from "./resources/token/token.schema";' : ''}
${auth ? 'import { tokenResolvers } from "./resources/token/token.resolvers";' : ''}
 ...
 */
createFile.prototype.resolverContextInterface = (auth) => {
...
import { RequestedFields } from "../graphql/ast/RequestedFields";
${auth ? 'import { AuthExample } from \'./AuthExampleInterface\';' : ''}

export interface ResolverContext {
    db?: DbConnection;
    ${auth ? 'authorization?: String;' : ''}
    ${auth ? 'authExample?: AuthExample;' : ''}
...
createFile.prototype.createAuthExampleInterface = (projName) => {
    const authExampleInterface = `export interface AuthExample {
    id: number;
    email?: string;
}
`;

    const path = projName ? `./${__projName}/src/interfaces/AuthExampleInterface.ts` : `./src/interfaces/AuthExampleInterface.ts`;
    fs.writeFile(path, authExampleInterface, (err) => {if (err) throw err;});
}

createFile.prototype.createExtractJwtMiddleware = (projName) => {
    const extractJwtMiddleware = `import * as jwt from 'jsonwebtoken'
...
export const extractJwtMiddleware = (): RequestHandler => {
    return (req: Request, res: Response, next: NextFunction): void => {
        ...
        jwt.verify(token, JWT_SECRET, (err, decoded: any) => {
            ...
            db.Example.findById(decoded.sub, {
                attributes: ['id', 'email']
            }).then((example: ExampleInstance) => {
                if(example) {
                    req['context']['authExample'] = {
                        id: example.get('id'),
...
`;
    ...
    const path = projName ? `./${__projName}/src/middlewares/extract-jwt.middleware.ts` : `./src/middlewares/extract-jwt.middleware.ts`;
    fs.writeFile(path, extractJwtMiddleware, (err) => {if (err) throw err;});
}

createFile.prototype.createAuthResolver = (projName) => {
    const auth = `import { GraphQLFieldResolver } from "graphql";
...
export const authResolver: ComposableResolver<any, ResolverContext> = 
    (resolver: GraphQLFieldResolver<any, ResolverContext>): GraphQLFieldResolver<any, ResolverContext> => {
        return (parent, args, context: ResolverContext, info) => {
            if (context.authExample || context.authorization) {
...
`;
    ...
    const path = projName ? `./${__projName}/src/graphql/composable/auth.resolver.ts` : `./src/graphql/composable/auth.resolver.ts`;
    fs.writeFile(path, auth, (err) => {if (err) throw err;});
}

createFile.prototype.createVerifyTokenResolver = (projName) => {
    const verifyToken = `import { GraphQLFieldResolver } from "graphql";
...
`;
    ...
    const path = projName ? `./${__projName}/src/graphql/composable/verify-token.resolver.ts` : `./src/graphql/composable/verify-token.resolver.ts`;
    fs.writeFile(path, verifyToken, (err) => {if (err) throw err;});
}

createFile.prototype.createTokenResolver = (projName) => {
    const tokenResolver = `import * as jwt from 'jsonwebtoken';
...
export const tokenResolvers = {
    Mutation: {
        ...
        return db.Example.findOne({
            where: { email: email },
            attributes: ['id', 'password']
        }).then((example: ExampleInstance) => {
            let errorMessage: string = 'Unauthorized, wrong email or password!';
            if(!example || !example.isPassword(example.get('password'), password)) { throw new Error(errorMessage) }

            const payload = { sub: example.get('id') };
...
`;
    ...
    const path = projName ? `./${__projName}/src/graphql/resources/token/token.resolvers.ts` : `./src/graphql/resources/token/token.resolvers.ts`;
    fs.writeFile(path, tokenResolver, (err) => {if (err) throw err;});
}

createFile.prototype.createTokenSchema = (projName) => {
    const tokenSchema = `const tokenTypes = \`
    type Token {
        token: String!
    }
\`;
...
`;
    ...
    const path = projName ? `./${__projName}/src/graphql/resources/token/token.schema.ts` : `./src/graphql/resources/token/token.schema.ts`;
    fs.writeFile(path, tokenSchema, (err) => {if (err) throw err;});
}
