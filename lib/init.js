...
function init(projName, dialect) {
    if (dialect === 'mysql' || dialect === 'mssql' || dialect === 'sqlite' || dialect === 'postgres') {
            ...
            if (!stderr) {
                ...
                exec(`mkdir ${projName}`, (err, stdout, stderr) => {
                    if (!stderr) {
                        ...
                        var creatingFiles = new createFile(projName, dialect);
        ...
        console.log(colors.error('The seted dialect not exists. Possible values: (mysql, mssql, sqlite, postgres)'))
    }
}
